...
| `manager_add({email})` | Make somebody a co-manager, and commit. **Manager-gated.** Identified by email address; a username is refused. Refused unless that person has added a working key to this project themselves, checked with the provider's list-models call, which spends nothing. |
...
| `manager_transfer({email, step_down?})` | Hand the primary manager role over, and commit. **Primary only.** The new primary needs a working key of their own on this project, checked first. The outgoing primary stays a co-manager unless `step_down`, which is refused while the project's lent GitHub access is still theirs. |
